...
context "PHP FPM" do
  describe "index.php" do
    let(:subject) { command("curl -i php-static.dev") }
    ...
    it "contains the correct information" do
      expect(subject.stdout).to match /^Nginx is serving PHP #{Regexp.quote(@php_version)} code on php-static\.dev$/
    end
  end

  describe "another url" do
    let(:subject) { command("curl -i php-static.dev/some_other_url") }
